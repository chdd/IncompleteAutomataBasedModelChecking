...
public class IntersectionBuilderRemoveIntersectionStateTest {
	...
	/**
	 * Test method for
	 * {@link it.polimi.checker.intersection.IntersectionBuilder#removeIntersectionState(State)}
	 * .
	 */
	@Test
	public void testUpdateIntersection() {
		...
		assertEquals(2, intersectionBuilder
				.getIntersectionTransitionsBlackBoxStatesMap().keySet().size());

		assertTrue(intersectionBuilder
				.getIntersectionTransitionsBlackBoxStatesMap().values().size() == 2);

		State intersectionState = intersectionBuilder.getIntersectionState(
				modelState2, claimState2, 0);
		intersectionBuilder.removeIntersectionState(intersectionState);

		assertEquals(1, intersectionBuilder
				.getIntersectionTransitionsBlackBoxStatesMap().keySet().size());

		intersectionState = intersectionBuilder.getIntersectionState(
				modelState2, claimState3, 0);
		intersectionBuilder.removeIntersectionState(intersectionState);

		assertEquals(0, intersectionBuilder
				.getIntersectionTransitionsBlackBoxStatesMap().keySet().size());
	 ...
	 */
	@Test
	public void testUpdateIntersection2() {
		this.model.addInitialState(modelState1);
		this.model.addBlackBoxState(modelState2);
		this.model.addAcceptState(modelState3);
