...
public class TestInjectorShould {
	...
	public void notRemoveTransparentStatesFromTheIBA() throws Exception {
		...
		Set<State> transparentStates = new HashSet<State>(
				model.getTransparentStates());
		transparentStates.remove(replacement.getModelState());
		assertTrue(result.getTransparentStates().containsAll(transparentStates));
	}
	...
	public void insertTheTransparentReplacementStatesIntoTheIBA() throws Exception {
		assertTrue(result.getTransparentStates().containsAll(
				replacement.getAutomaton().getTransparentStates()));
	}
	...
	public void injectTheIncomingTransitionsOfTheReplacement() throws Exception {
		for(PluggingTransition plugTransition: replacement.getIncomingPorts()){
	...
	private IBA createSendModel() {
		...
		model.addTransparentState(send1);
		model.addTransparentState(send2);
	...
	private Replacement createSend1ReplacementModel() {
		...
		automaton.addTransparentState(state17);
