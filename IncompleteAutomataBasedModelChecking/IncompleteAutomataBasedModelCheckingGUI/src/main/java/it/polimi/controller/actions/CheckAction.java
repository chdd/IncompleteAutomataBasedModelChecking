...
import it.polimi.constraints.brzozowski.Brzozowski;
...
implements ActionInterface<CONSTRAINEDELEMENT, STATE, TRANSITION> {
	...
	perform(ModelInterface<CONSTRAINEDELEMENT, STATE, TRANSITION, INTERSECTIONSTATE, 
		...
		System.out.println(model.getVerificationResults().getRegularExpression().getLogicalItem().simplify().toString());
	}
