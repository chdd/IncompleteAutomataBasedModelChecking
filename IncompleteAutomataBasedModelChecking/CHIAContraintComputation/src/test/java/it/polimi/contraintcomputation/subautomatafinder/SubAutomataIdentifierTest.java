...
import it.polimi.contraintcomputation.abstractor.AbstractorTest;
...
public class SubAutomataIdentifierTest {
	...
	public void testAbstractIntersection() {
		...
		SubAutomataIdentifier<State, Transition> identifier = new SubAutomataIdentifier<State, Transition>(
				...
				new TransitionFactoryIntersectionImpl<Component<State, Transition>>(
						Transition.class),
				new TransitionFactoryIntersectionImpl<State>(Transition.class));

		AbstractedBA<State, Transition, Component<State, Transition>> simplifiedAutomata =identifier.getSubAutomata();
		
		
		assertTrue(simplifiedAutomata.getStates().size() == 5);
		assertTrue(simplifiedAutomata.getInitialStates().size() == 1);
		assertTrue(simplifiedAutomata.getAcceptStates().size() == 1);
		assertTrue(simplifiedAutomata.getOutTransitions(
				simplifiedAutomata.getInitialStates().iterator().next()).size() == 1);
	
		for(Component<State, Transition> c: simplifiedAutomata.getStates()){
			if(c.getName().equals("send1")){
